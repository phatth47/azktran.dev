...
const CodeLine: React.FC<{ content: string }> = ({ content }) => {
  ...
  const addStyledText = (text: string, className: string) => {
    parts.push(<span key={currentIndex} className={className}>{text}</span>);
    currentIndex++;
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg flex flex-col h-full overflow-hidden">
        ...
        <div className="flex justify-between items-center p-4">
          <div className="flex space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${selectedTab === 'entity'
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${selectedTab === 'model'
          ...
          <div className="flex flex-wrap gap-2">
            {generatedCodes.map(({ name, fileName }) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded transition-colors ${selectedFile === name
      ...
      <div className="flex-1 min-h-0">
        {selectedFile && (
          <div className="h-full overflow-auto bg-gray-50 dark:bg-gray-900 p-4">
            <div className="relative">
              {/* Copy Button with Notification */}
              <div className="absolute top-2 right-2 z-50"> {/* Increased z-index and used container */}
                <button
                ...
                {showCopyNotification && (
                  <div className="absolute -top-8 right-0 px-2 py-1 text-xs text-white 
                         bg-gray-800 rounded shadow-lg whitespace-nowrap z-50">
                    Copied!
                  </div>
                )}
              </div>

              <pre className="text-sm font-mono leading-normal">
